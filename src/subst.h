...
class Subst {
 public:
  ...
  static Subst makeSimpleAssignment(const std::vector<TypedVar> &variables,
                                    std::vector<Expr> &&values);
  static Subst makeSelect(std::vector<std::pair<Pred, Subst>> &&clauses);
  static Subst makeSelectElse(std::vector<std::pair<Pred, Subst>> &&clauses,
                              Subst &&els);
  static Subst makeCase(Expr &&e, std::vector<CaseChoice> &&cases);
  static Subst makeCaseElse(Expr &&e, std::vector<CaseChoice> &&cases,
                            Subst &&els);
  static Subst makeAny(const std::vector<TypedVar> &vars, Pred &&p,
                       Subst &&body);
  static Subst makeWitness(std::map<std::string, Expr> &&witnesses,
                           Subst &&body);
  static Subst makeOpCall(const std::string &name, std::vector<Expr> &&input,
                          const std::vector<TypedVar> &output,
                          const std::vector<TypedVar> &op_input,
                          const std::vector<TypedVar> &op_output,
                          Pred &&op_precondition, Subst &&op_body);
  ...
  class Visitor {
   public:
    ...
    virtual void visitIfThenElse(const Pred &p, const Subst &s_if,
                                 const Subst &s_else) = 0;
    virtual void visitSimpleAssignment(const std::vector<TypedVar> &variables,
                                       const std::vector<Expr> &values) = 0;
    virtual void visitSelect(
        const std::vector<std::pair<Pred, Subst>> &clauses) = 0;
    virtual void visitSelectElse(
        const std::vector<std::pair<Pred, Subst>> &clauses,
        const Subst &els) = 0;
    virtual void visitCase(const Expr &e,
                           const std::vector<CaseChoice> &cases) = 0;
    virtual void visitCaseElse(const Expr &e,
                               const std::vector<CaseChoice> &cases,
                               const Subst &els) = 0;
    virtual void visitAny(const std::vector<TypedVar> &vars, const Pred &p,
                          const Subst &body) = 0;
    virtual void visitOpCall(const std::string &name,
                             const std::vector<Expr> &input,
                             const std::vector<TypedVar> &output,
                             const std::vector<TypedVar> &op_input,
                             const std::vector<TypedVar> &op_output,
                             const Pred &op_precondition,
                             const Subst &op_body) = 0;
    virtual void visitWhile(const Pred &cond, const Subst &body,
                            const Pred &inv, const Expr &var) = 0;
    ...
    virtual void visitWitness(const std::map<std::string, Expr> &witnesses,
                              const Subst &body) = 0;
  };
  ...
  class SubstDesc {
   public:
    ...
    virtual void getModifiedVars(const std::set<VarName> &boundVars,
  ...
  void getModifiedVars(const std::set<VarName> &boundVars,
  ...
  // Get all identifiers occuring free in the body (or precondition) of the
  // operations called in the substitution.
 ...
 private:
  ...
  std::unique_ptr<SubstDesc> m_desc;  // the content of the substitution (may be
                                      // null if the substitution is Skip).
  // Constructor
...
class Subst::AssertSubst : public SubstDesc {
 public:
  // Constructor
  AssertSubst(Pred &&c, Subst &&s)
      : condition{std::move(c)}, content{std::move(s)} {};
  // Members
  Pred condition;
  Subst content;
  // Methods
...
class Subst::WitnessSubst : public SubstDesc {
 public:
  ...
  WitnessSubst(std::map<std::string, Expr> &&witnesses, Subst &&body)
      : witnesses{std::move(witnesses)}, body{std::move(body)} {}
  // Members
  ...
  size_t hash_combine(size_t seed) const {
    for (auto &p : witnesses)
      seed =
          hashUtil::hash_combine_string(p.first, p.second.hash_combine(seed));
    return body.hash_combine(seed);
  }
  void getFreeVars(const std::set<VarName> &boundVars,
                   std::set<VarName> &accu) const {
    for (auto &p : witnesses) p.second.getFreeVars(boundVars, accu);
    body.getFreeVars(boundVars, accu);
  }
  void getAllVars(std::set<VarName> &accu) const {
    for (auto &p : witnesses) p.second.getAllVars(accu);
  ...
  void alpha(const std::map<VarName, VarName> &map) {
    for (auto &p : witnesses) p.second.alpha(map);
    body.alpha(map);
  }
  void getInnerFreeVars(std::set<VarName> &accu) const {
  ...
  void substFreshId(const std::string &id, const VarName &v) {
    for (auto &p : witnesses) p.second.substFreshId(id, v);
    body.substFreshId(id, v);
  }
  SubstDesc *copy() const {
    std::map<std::string, Expr> witnesses2;
    for (auto &p : witnesses) witnesses2[p.first] = p.second.copy();
...
class Subst::SimpleAssignmentSubst : public SubstDesc {
 public:
  // Constructor
  SimpleAssignmentSubst(const std::vector<TypedVar> &vars,
  ...
  size_t hash_combine(size_t seed) const {
    for (auto &v : vars) seed = v.hash_combine(seed);
    for (auto &e : exprs) seed = e.hash_combine(seed);
    return seed;
  }
  void getFreeVars(const std::set<VarName> &boundVars,
                   std::set<VarName> &accu) const {
    for (auto &v : vars) {
      if (boundVars.find(v.name) != boundVars.end()) accu.insert(v.name);
    }
    for (auto &e : exprs) e.getFreeVars(boundVars, accu);
  }
  void getAllVars(std::set<VarName> &accu) const {
    for (auto &v : vars) accu.insert(v.name);
    for (auto &e : exprs) e.getAllVars(accu);
  }
  void getModifiedVars(const std::set<VarName> &boundVars,
                       std::set<TypedVar> &accu) const {
    for (auto &v : vars) {
      if (boundVars.find(v.name) == boundVars.end()) accu.insert(v);
    }
  }
  void alpha(const std::map<VarName, VarName> &map) {
    for (size_t i = 0; i < vars.size(); i++) {
      auto it = map.find(vars[i].name);
      if (it != map.end()) vars[i].name = it->second;
    }
    for (auto &e : exprs) e.alpha(map);
  }
  void getInnerFreeVars(std::set<VarName> &) const {}
  void substFreshId(const std::string &id, const VarName &v) {
    for (size_t i = 0; i < vars.size(); i++) {
      if (vars[i].name.kind() == VarName::Kind::FreshId &&
          vars[i].name.prefix() == id)
        vars[i].name = v;
    }
    for (auto &e : exprs) e.substFreshId(id, v);
  }
  SubstDesc *copy() const {
    std::vector<Expr> exprs2;
    for (auto &e : exprs) exprs2.push_back(e.copy());
...
class Subst::SelectSubst : public SubstDesc {
 public:
  ...
  void getModifiedVars(const std::set<VarName> &boundVars,
                       std::set<TypedVar> &accu) const {
    for (auto &p : clauses) p.second.getModifiedVars(boundVars, accu);
  }
  void alpha(const std::map<VarName, VarName> &map) {
    for (auto &p : clauses) {
      p.first.alpha(map);
  ...
  void getInnerFreeVars(std::set<VarName> &accu) const {
    for (auto &p : clauses) p.second.getInnerFreeVars(accu);
  }
  void substFreshId(const std::string &id, const VarName &v) {
    for (auto &p : clauses) {
 ...
 public:
  ...
  size_t hash_combine(size_t seed) const {
    for (auto &ch : m_cases) {
      ...
      for (auto &e : ch.values) seed = e.hash_combine(seed);
    }
  ...
  void getFreeVars(const std::set<VarName> &boundVars,
    ...
    for (auto &ch : m_cases) {
      for (auto &e : ch.values) e.getFreeVars(boundVars, accu);
  ...
  void getAllVars(std::set<VarName> &accu) const {
    ...
    for (auto &ch : m_cases) {
      for (auto &e : ch.values) e.getAllVars(accu);
      ch.body.getAllVars(accu);
    }
  }
  void getModifiedVars(const std::set<VarName> &boundVars,
                       std::set<TypedVar> &accu) const {
    for (auto &ch : m_cases) ch.body.getModifiedVars(boundVars, accu);
  }

  void alpha(const std::map<VarName, VarName> &map) {
    m_expr.alpha(map);
    for (auto &ch : m_cases) {
      for (auto &e : ch.values) e.alpha(map);
      ch.body.alpha(map);
    }
  }
  void getInnerFreeVars(std::set<VarName> &accu) const {
    for (auto &ch : m_cases) ch.body.getInnerFreeVars(accu);
  }
  void substFreshId(const std::string &id, const VarName &v) {
    m_expr.substFreshId(id, v);
    for (auto &ch : m_cases) {
      for (auto &e : ch.values) e.substFreshId(id, v);
  ...
  SubstDesc *copy() const {
    ...
    for (auto &ch : m_cases) {
      CaseChoice ch2;
      for (auto &e : ch.values) ch2.values.push_back(e.copy());
...
class Subst::AnySubst : public SubstDesc {
 public:
  ...
  void getFreeVars(const std::set<VarName> &boundVars,
    ...
    for (auto &v : vars) boundVars2.insert(v.name);
  ...
  void alpha(const std::map<VarName, VarName> &map) {
    ...
    for (auto &v : vars) {
      if (map.find(v.name) != map.end()) map2.erase(v.name);
    }
    if (!map2.empty()) {
      p.alpha(map2);
      body.alpha(map2);
  ...
  SubstDesc *copy() const { return new AnySubst(vars, p.copy(), body.copy()); }
};

class Subst::OpCallSubst : public SubstDesc {
 public:
  // Constructor
  OpCallSubst(const std::string &name, std::vector<Expr> &&input,
              const std::vector<TypedVar> &output,
              const std::vector<TypedVar> &op_input,
              const std::vector<TypedVar> &op_output, Pred &&op_precondition,
              Subst &&op_body)
      : name{name},
  ...
  // Methods
  size_t hash_combine(size_t seed) const {
    for (auto &e : input) seed = e.hash_combine(seed);
    for (auto &v : output) seed = v.hash_combine(seed);
    for (auto &v : op_input) seed = v.hash_combine(seed);
    for (auto &v : op_output) seed = v.hash_combine(seed);
    return hashUtil::hash_combine_string(
        name, op_precondition.hash_combine(op_body.hash_combine(seed)));
  }
  void getFreeVars(const std::set<VarName> &boundVars,
                   std::set<VarName> &accu) const {
    for (auto &v : output) {
    ...
    for (auto &e : input) e.getFreeVars(boundVars, accu);
    std::set<VarName> boundVars2;
    for (auto &v : op_input) boundVars2.insert(v.name);
    for (auto &v : op_output) boundVars2.insert(v.name);
    op_precondition.getFreeVars(boundVars2, accu);
    op_body.getFreeVars(boundVars2, accu);
  }
  void getAllVars(std::set<VarName> &accu) const {
    for (auto &v : output) accu.insert(v.name);
    for (auto &e : input) e.getAllVars(accu);
    for (auto &v : op_input) accu.insert(v.name);
    for (auto &v : op_output) accu.insert(v.name);
  ...
  void alpha(const std::map<VarName, VarName> &map) {
    for (size_t i = 0; i < output.size(); i++) {
      ...
      if (it != map.end()) output[i].name = it->second;
    }
    for (auto &e : input) e.alpha(map);
  }
  void getInnerFreeVars(std::set<VarName> &accu) const {
    std::set<VarName> boundVars;
    for (auto &v : op_input) boundVars.insert(v.name);
    for (auto &v : op_output) boundVars.insert(v.name);
    op_precondition.getFreeVars(boundVars, accu);
    op_body.getFreeVars(boundVars, accu);
  }
  void substFreshId(const std::string &id, const VarName &v) {
    for (size_t i = 0; i < output.size(); i++) {
      if (output[i].name.kind() == VarName::Kind::FreshId &&
          output[i].name.prefix() == id)
        output[i].name = v;
    }
    for (auto &e : input) e.substFreshId(id, v);
    for (size_t i = 0; i < op_output.size(); i++) {
      if (op_output[i].name.kind() == VarName::Kind::FreshId &&
          op_output[i].name.prefix() == id)
        op_output[i].name = v;
    }
    for (size_t i = 0; i < op_input.size(); i++) {
      if (op_input[i].name.kind() == VarName::Kind::FreshId &&
          op_input[i].name.prefix() == id)
        op_input[i].name = v;
    }
  ...
  SubstDesc *copy() const {
    ...
    for (auto &e : input) input2.push_back(e.copy());
    return new OpCallSubst(name, std::move(input2), output, op_input, op_output,
                           op_precondition.copy(), op_body.copy());
  }
};
 ...
 public:
  ...
  size_t hash_combine(size_t seed) const {
    return cond.hash_combine(
        body.hash_combine(inv.hash_combine(var.hash_combine(seed))));
  }
  void getFreeVars(const std::set<VarName> &boundVars,
                   std::set<VarName> &accu) const {
