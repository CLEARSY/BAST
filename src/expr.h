...
class Expr {
    public:
        ...
        static constexpr size_t binaryOpCount = static_cast<size_t>(BinaryOp::Arity) + 1;

        enum class TernaryOp {
            Son, Bin
        };
        static std::string to_string(TernaryOp op);
        static constexpr size_t ternaryOpCount = static_cast<size_t>(TernaryOp::Bin) + 1;

        enum class QuantifiedOp {
            Lambda, Intersection, Union, ISum, IProduct, RSum, RProduct
        };
        static std::string to_string(QuantifiedOp op);
        static constexpr size_t quantifiedOpCount = static_cast<size_t>(QuantifiedOp::RProduct) + 1;
...
class Expr::Visitor {
    ...
    static constexpr size_t eConstantCount = static_cast<size_t>(EConstant::Predecessor) + 1;
