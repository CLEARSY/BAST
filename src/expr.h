...
class Expr {
 public:
  ...
  static constexpr size_t binaryOpCount =
      static_cast<size_t>(BinaryOp::Arity) + 1;

  enum class TernaryOp { Son, Bin };
  static std::string to_string(TernaryOp op);
  static constexpr size_t ternaryOpCount =
      static_cast<size_t>(TernaryOp::Bin) + 1;
  ...
  static constexpr size_t quantifiedOpCount =
      static_cast<size_t>(QuantifiedOp::RProduct) + 1;
  ...
  static constexpr size_t unaryOpCount = static_cast<size_t>(UnaryOp::Bin) + 1;

  enum class NaryOp { Sequence, Set };
  static std::string to_string(NaryOp op);
  static constexpr size_t naryOpCount = static_cast<size_t>(NaryOp::Set) + 1;

  Expr() : tag{EKind::MaxInt}, desc{nullptr}, type{BType::INT}, bxmlTag{} {};
  ...
  struct Decimal {
    Decimal(const std::string &integerPart, const std::string &fractionalPart)
      ...
      assert(fractionalPart == "0" ||
             fractionalPart[fractionalPart.size() - 1] != '0');
    ...
    Decimal(const std::string &integerPart) : Decimal(integerPart, "0") {};
    std::string integerPart;
    std::string fractionalPart;
    int compare(const Decimal &other) const;
  };

  static Expr makeInteger(const std::string &i,
                          const std::vector<std::string> &bxmlTag = {});
  static Expr makeString(const std::string &s,
                         const std::vector<std::string> &bxmlTag = {});
  static Expr makeReal(const Decimal &d,
                       const std::vector<std::string> &bxmlTag = {});
  ...
  static Expr makeBinaryExpr(BinaryOp op, Expr &&lhs, Expr &&rhs,
                             const BType &type,
                             const std::vector<std::string> &bxmlTag = {});
  static Expr makeUnaryExpr(UnaryOp op, Expr &&lhs, const BType &type,
                            const std::vector<std::string> &bxmlTag = {});
  static Expr makeNaryExpr(NaryOp op, std::vector<Expr> &&vec,
                           const BType &type,
                           const std::vector<std::string> &bxmlTag = {});
  static Expr makeTernaryExpr(TernaryOp op, Expr &&fst, Expr &&snd, Expr &&thd,
                              const BType &type,
                              const std::vector<std::string> &bxmlTag = {});
  static Expr makeBooleanExpr(Pred &&p,
                              const std::vector<std::string> &bxmlTag = {});
  static Expr makeRecord(std::vector<std::pair<std::string, Expr>> &&fds,
                         const BType &type,
                         const std::vector<std::string> &bxmlTag =
                             {});  // /!\ fields must be sorted alphabetically
  static Expr makeStruct(std::vector<std::pair<std::string, Expr>> &&fds,
                         const BType &type,
                         const std::vector<std::string> &bxmlTag =
                             {});  // /!\ fields must be sorted alphabetically
  static Expr makeQuantifiedExpr(QuantifiedOp op,
                                 const std::vector<TypedVar> vars, Pred &&cond,
                                 Expr &&body, const BType &type,
                                 const std::vector<std::string> &bxmlTag = {});
  static Expr makeQuantifiedSet(const std::vector<TypedVar> vars, Pred &&cond,
                                const BType &type,
                                const std::vector<std::string> &bxmlTag = {});
  static Expr makeRecordFieldUpdate(
      Expr &&rec, const std::string &label, Expr &&value, const BType &type,
      const std::vector<std::string> &bxmlTag = {});
  static Expr makeRecordFieldAccess(
      Expr &&rec, const std::string &label, const BType &type,
      const std::vector<std::string> &bxmlTag = {});
   ...
   * \param freeVarsThis receives the set of free variables occuring in
   * expression that are not in freeVars.
   */
  ...
  // Remarque 2: ne permet pas de comparer numériquement (les littéraux entiers
  // ou décimaux)
  static int compare(const Expr &lhs, const Expr &rhs);
  static int vec_compare(const std::vector<Expr> &lhs,
  ...
  // Attention il ne faut pas utiliser ces opérateurs pour comparer des
  // expressions numériques à des fins de simplification.
  // inline bool operator==(const Expr& other) const { return
  // compare(*this,other) == 0; } inline bool operator!=(const Expr& other)
  ...
  // Expr& other) const { return compare(*this,other) >= 0; }
  ...
  static bool isRenamingNeeded(const std::vector<TypedVar> &vars,
                               const std::set<VarName> freeVars);
  static void renameVars(std::vector<TypedVar> &vars,
                         const std::set<VarName> freeVars,
 ...
 private:
  class ExprDesc {
   public:
    ...
    virtual void getFreeVars(const std::set<VarName> &bv,
                             const std::set<VarName> &freeVars,
  ...
  // Constructor
  Expr(EKind tag, ExprDesc *desc, const BType &ty,
       const std::vector<std::string> &bxmlTag);
};
...
class Expr::Visitor {
 public:
  ...
  static constexpr size_t eConstantCount =
      static_cast<size_t>(EConstant::Predecessor) + 1;
  ...
  virtual void visitUnaryExpression(const BType &type,
                                    const std::vector<std::string> &bxmlTag,
                                    Expr::UnaryOp op, const Expr &e) = 0;
  ...
  virtual void visitTernaryExpression(const BType &type,
                                      ...
                                      const Expr &snd, const Expr &thd) = 0;
  virtual void visitNaryExpression(const BType &type,
                                   const std::vector<std::string> &bxmlTag,
                                   Expr::NaryOp op,
                                   const std::vector<Expr> &vec) = 0;
  ...
  virtual void visitRecord(
      ...
      const std::vector<std::pair<std::string, Expr>> &fds) = 0;
  ...
  virtual void visitQuantifiedExpr(const BType &type,
                                   ...
                                   const Pred &cond, const Expr &body) = 0;
  ...
  virtual void visitRecordAccess(const BType &type,
                                 const std::vector<std::string> &bxmlTag,
                                 const Expr &rec, const std::string &label) = 0;
};
