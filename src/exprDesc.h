...
class Expr::UnaryExpr : public Expr::ExprDesc {
 public:
  // Constructor
  UnaryExpr(UnaryOp op, Expr &&e) : op{op}, content{std::move(e)} {};
  // Members
  const UnaryOp op;
  Expr content;
  // Methods
  ...
  UnaryExpr *copy() const { return new UnaryExpr(op, content.copy()); }
  void subst(const std::map<VarName, Expr> &map) { content.subst(map); }
...
class Expr::BinaryExpr : public Expr::ExprDesc {
 public:
  ...
  size_t hash_combine(size_t seed) const {
    return hashUtil::hash_combine_int(static_cast<int>(op),
                                      lhs.hash_combine(rhs.hash_combine(seed)));
  }
...
class Expr::TernaryExpr : public Expr::ExprDesc {
 public:
  ...
  TernaryExpr(TernaryOp op, Expr &&fst, Expr &&snd, Expr &&thd)
      : op{op}, fst{std::move(fst)}, snd{std::move(snd)}, thd{std::move(thd)} {
        };
  // Members
  const TernaryOp op;
  Expr fst;
  ...
  size_t hash_combine(size_t seed) const {
    return hashUtil::hash_combine_int(
        static_cast<int>(op),
        fst.hash_combine(snd.hash_combine(thd.hash_combine(seed))));
  }
...
class Expr::NaryExpr : public Expr::ExprDesc {
 public:
  // Constructor
  NaryExpr(NaryOp op, std::vector<Expr> &&vec) : op{op}, vec{std::move(vec)} {};
  // Members
  const NaryOp op;
  std::vector<Expr> vec;
  // Methods
  size_t hash_combine(size_t seed) const {
    ...
    for (auto &e : vec) seed = e.hash_combine(seed);
    return seed;
  }
  NaryExpr *copy() const {
    std::vector<Expr> vec2;
    for (auto &p : vec) vec2.push_back(p.copy());
    return new NaryExpr(op, std::move(vec2));
  }
  void subst(const std::map<VarName, Expr> &map) {
    for (auto &e : vec) e.subst(map);
  }
  void alpha(const std::map<VarName, VarName> &map) {
    for (auto &e : vec) e.alpha(map);
  }
  void getAllVars(std::set<VarName> &accu) const {
    for (auto &e : vec) e.getAllVars(accu);
  };
  void getFreeVars(const std::set<VarName> &boundVars,
                   const std::set<VarName> &freeVars,
                   std::set<VarName> &freeVarsThis) const {
    for (auto &e : vec) e.getFreeVars(boundVars, freeVars, freeVarsThis);
  };
  void getFreeVars(const std::set<VarName> &boundVars,
                   std::set<VarName> &accu) const {
    for (auto &e : vec) e.getFreeVars(boundVars, accu);
  };
  void substFreshId(const std::string &id, const VarName &v) {
    for (auto &e : vec) e.substFreshId(id, v);
  }
  void getFreeTVars(const std::set<VarName> &boundVars,
                    std::set<TypedVar> &accu, const BType &) const {
    for (auto &e : vec) e.getFreeTVars(boundVars, accu);
  };
};

extern struct RecordFieldCmp {
  bool operator()(const std::pair<std::string, Expr> &a,
                  const std::pair<std::string, Expr> &b) {
...
class Expr::RecordExpr : public Expr::ExprDesc {
 public:
  ...
  RecordExpr *copy() const {
    ...
    for (auto &p : fields) fields2.push_back({p.first, p.second.copy()});
    return new RecordExpr(std::move(fields2));
  }
  void subst(const std::map<VarName, Expr> &map) {
    for (auto &p : fields) p.second.subst(map);
  }
  void alpha(const std::map<VarName, VarName> &map) {
    for (auto &p : fields) p.second.alpha(map);
  }
  void getAllVars(std::set<VarName> &accu) const {
    for (auto &pair : fields) pair.second.getAllVars(accu);
  };
  void getFreeVars(const std::set<VarName> &boundVars,
                   const std::set<VarName> &freeVars,
                   std::set<VarName> &freeVarsThis) const {
  ...
  void getFreeVars(const std::set<VarName> &boundVars,
                   std::set<VarName> &accu) const {
    for (auto &pair : fields) pair.second.getFreeVars(boundVars, accu);
  };
  void substFreshId(const std::string &id, const VarName &v) {
    for (auto &pair : fields) pair.second.substFreshId(id, v);
  }
  void getFreeTVars(const std::set<VarName> &boundVars,
                    std::set<TypedVar> &accu, const BType &) const {
    for (auto &pair : fields) pair.second.getFreeTVars(boundVars, accu);
  };
};
...
class Expr::QuantifiedSet : public Expr::ExprDesc {
 public:
  ...
  QuantifiedSet *copy() const { return new QuantifiedSet(vars, cond.copy()); }
  void subst(const std::map<VarName, Expr> &map) {
    std::map<VarName, Expr> map2;
    for (auto &p : map) map2[p.first] = p.second.copy();
    for (auto &v : vars) map2.erase(v.name);
    std::set<VarName> freeVars;
    for (auto &p : map) p.second.getFreeVars({}, freeVars);
...
class Expr::QuantifiedExpr : public Expr::ExprDesc {
 public:
  // Constructor
  QuantifiedExpr(QuantifiedOp op, const std::vector<TypedVar> &vars,
                 Pred &&cond, Expr &&body)
      : op{op}, vars{vars}, cond{std::move(cond)}, body{std::move(body)} {};
  // Members
  ...
  size_t hash_combine(size_t seed) const {
    seed = hashUtil::hash_combine_int(static_cast<int>(op), seed);
    for (auto &v : vars) seed = v.hash_combine(seed);
    return cond.hash_combine(body.hash_combine(seed));
  }
...
class Expr::RecordAccessExpr : public Expr::ExprDesc {
 public:
  ...
  void subst(const std::map<VarName, Expr> &map) { rec.subst(map); }
  void alpha(const std::map<VarName, VarName> &map) { rec.alpha(map); }
  void getAllVars(std::set<VarName> &accu) const { rec.getAllVars(accu); }
  void getFreeVars(const std::set<VarName> &boundVars,
                   const std::set<VarName> &freeVars,
                   std::set<VarName> &freeVarsThis) const {
...
class Expr::RecordUpdateExpr : public Expr::ExprDesc {
 public:
  ...
  RecordUpdateExpr(Expr &&rec, const std::string &label, Expr &&fval)
      : rec{std::move(rec)}, label{label}, fvalue{std::move(fval)} {};
  // Members
  Expr rec;
  const std::string label;
  Expr fvalue;
  // Methods
  size_t hash_combine(size_t seed) const {
    return rec.hash_combine(
        fvalue.hash_combine(hashUtil::hash_combine_string(label, seed)));
  }
