...
class Pred::NegationPred : public PredDesc {
 public:
  // Constructor
  NegationPred(Pred &&p) : operand{std::move(p)} {};
  // Members
  Pred operand;
  // Methods
  PKind tag() const { return PKind::Negation; }
  void accept(Visitor &v) const { v.visitNegation(operand); };
  size_t hash_combine(size_t seed) const { return operand.hash_combine(seed); }
  void subst(const std::map<VarName, Expr> &map) { operand.subst(map); };
  void alpha(const std::map<VarName, VarName> &map) { operand.alpha(map); };
  void getFreeVars(const std::set<VarName> &boundVars,
                   const std::set<VarName> &freeVars,
                   std::set<VarName> &freeVarsThis) const {
  ...
  void getAllVars(std::set<VarName> &accu) const { operand.getAllVars(accu); }
  void substFreshId(const std::string &id, const VarName &v) {
...
class Pred::Conjunction : public PredDesc {
 public:
  // Constructor
  Conjunction(std::vector<Pred> &&vec) : operands{std::move(vec)} {};
  // Members
  ...
  size_t hash_combine(size_t seed) const {
    for (auto &p : operands) seed = p.hash_combine(seed);
    return seed;
  }
  void subst(const std::map<VarName, Expr> &map) {
    for (auto &p : operands) p.subst(map);
  };
  void alpha(const std::map<VarName, VarName> &map) {
    for (auto &p : operands) p.alpha(map);
  };
  void getFreeVars(const std::set<VarName> &boundVars,
                   const std::set<VarName> &freeVars,
                   std::set<VarName> &freeVarsThis) const {
    for (auto &p : operands) p.getFreeVars(boundVars, freeVars, freeVarsThis);
  }
  void getFreeVars(const std::set<VarName> &boundVars,
                   std::set<VarName> &accu) const {
    for (auto &p : operands) p.getFreeVars(boundVars, accu);
  }
  void getAllVars(std::set<VarName> &accu) const {
    for (auto &p : operands) p.getAllVars(accu);
  }
  void substFreshId(const std::string &id, const VarName &v) {
    for (auto &p : operands) p.substFreshId(id, v);
  }
  void getFreeTVars(const std::set<VarName> &boundVars,
                    std::set<TypedVar> &accu) const {
    for (auto &p : operands) p.getFreeTVars(boundVars, accu);
  }
};
...
class Pred::Disjunction : public PredDesc {
 public:
  // Constructor
  Disjunction(std::vector<Pred> &&vec) : operands{std::move(vec)} {};
  // Members
  std::vector<Pred> operands;
  // Methods
...
class Pred::Exists : public PredDesc {
 public:
  ...
  Exists(const std::vector<TypedVar> &ids, Pred &&body)
      : vars{ids}, body{std::move(body)}, allowWitnessInstanciation{false} {};
  Exists(const std::vector<TypedVar> &ids, Pred &&body, bool witness)
      : vars{ids}, body{std::move(body)}, allowWitnessInstanciation{witness} {};
  // Members
  std::vector<TypedVar> vars;
  Pred body;
  ...
  size_t hash_combine(size_t seed) const {
    for (auto &v : vars) seed = v.hash_combine(seed);
    return hashUtil::hash_combine_int(allowWitnessInstanciation ? 1 : 0,
...
class Pred::True : public PredDesc {
 public:
  ...
  size_t hash_combine(size_t seed) const { return seed; }
