...
void Pred::getAllVars(std::set<VarName> &accu) const { desc->getAllVars(accu); }
void Pred::substFreshId(const std::string &id, const VarName &v) {
  desc->substFreshId(id, v);
}
void Pred::getFreeVars(const std::set<VarName> &boundVars,
...
void Pred::getFreeTVars(const std::set<VarName> &boundVars,
...
void Pred::alpha(const std::map<VarName, VarName> &map) { desc->alpha(map); }
...
int Pred::vec_compare(const std::vector<Pred> &lhs,
                      const std::vector<Pred> &rhs) {
...
std::string Pred::to_string(ComparisonOp op) {
  switch (op) {
    case ComparisonOp::Membership:
      return ":";
    case ComparisonOp::Subset:
      return "<:";
    case ComparisonOp::Strict_Subset:
      return "<<:";
    case ComparisonOp::Equality:
      return "=";
    case ComparisonOp::Fge:
      return ">=f";
    case ComparisonOp::Fgt:
      return ">f";
    case ComparisonOp::Flt:
      return "<f";
    case ComparisonOp::Fle:
      return "<=f";
    case ComparisonOp::Rle:
      return "<=r";
    case ComparisonOp::Rlt:
      return "<r";
    case ComparisonOp::Rge:
      return ">=r";
    case ComparisonOp::Rgt:
      return ">r";
    case ComparisonOp::Ile:
      return "<=i";
    case ComparisonOp::Ilt:
      return "<i";
    case ComparisonOp::Ige:
      return ">=i";
    case ComparisonOp::Igt:
      return ">i";
  }
  assert(false);  // unreachable
  return "";
...
std::string Pred::show() const {
  switch (desc->tag()) {
    ...
    case Pred::PKind::Conjunction: {
      ...
      std::string accu;
      for (auto &p : prd.operands) accu += " " + p.show();
      return "(and" + accu + ")";
    }
    case Pred::PKind::Disjunction: {
...
bool Pred::alpha_equals(Context &ctx, const Pred &p1, const Pred &p2) {
  ...
  switch (p1.getTag()) {
    case Pred::PKind::Implication: {
      auto &b1 = p1.toImplication();
      auto &b2 = p2.toImplication();
      return alpha_equals(ctx, b1.lhs, b2.lhs) &&
    ...
    case Pred::PKind::ExprComparison: {
      ...
      return c1.op == c2.op && Expr::alpha_equals(ctx, c1.lhs, c2.lhs) &&
             Expr::alpha_equals(ctx, c1.rhs, c2.rhs);
    }
    case Pred::PKind::Negation: {
      return alpha_equals(ctx, p1.toNegation().operand,
                          p2.toNegation().operand);
    }

    case Pred::PKind::Conjunction: {
      auto &prd1 = p1.toConjunction();
      auto &prd2 = p2.toConjunction();
      if (prd1.operands.size() != prd2.operands.size()) return false;
    ...
    case Pred::PKind::Forall: {
      auto &q1 = p1.toForall();
      auto &q2 = p2.toForall();
      if (!ctx.push(q1.vars, q2.vars)) return false;
