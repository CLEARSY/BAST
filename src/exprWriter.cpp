...
bool compareTypesUnfold(const BType ty1, const BType ty2) {
  ...
  switch (ty1.getKind()) {
    ...
    case BType::Kind::PowerType:
      return compareTypesUnfold(ty1.toPowerType().content,
                                ty2.toPowerType().content);
    case BType::Kind::ProductType:
      return compareTypesUnfold(ty1.toProductType().lhs,
                                ty2.toProductType().lhs) &&
             compareTypesUnfold(ty1.toProductType().rhs,
                                ty2.toProductType().rhs);
    case BType::Kind::AbstractSet:
      return ty1.toAbstractSetType().getName().compare(
                 ty2.toAbstractSetType().getName()) == 0;
    case BType::Kind::EnumeratedSet:
      return ty1.toEnumeratedSetType().getName().compare(
                 ty2.toEnumeratedSetType().getName()) == 0;
    default:
      return false;
  }
}
...
void writeExprAttributes(const BType &type,
                         const std::vector<std::string> &bxmlTag,
                         tinyxml2::XMLPrinter &stream, TypeMap_t &typeInfos) {
  stream.PushAttribute("typref",
                       std::to_string(getTypRef(typeInfos, type)).c_str());
  if (!bxmlTag.empty()) {
    std::string result = std::accumulate(
        std::next(bxmlTag.begin()), bxmlTag.end(), bxmlTag[0],
...
void writeTypedVar(tinyxml2::XMLPrinter &stream, TypeMap_t &typeInfos,
  ...
  stream.PushAttribute("typref",
                       std::to_string(getTypRef(typeInfos, v.type)).c_str());
  stream.CloseElement();  // Id
}

class ExprWriterVisitor : public Expr::Visitor {
 public:
  void visitConstant(const BType &type, const std::vector<std::string> &bxmlTag,
  ...
  void visitIntegerLiteral(const BType &type,
                           const std::vector<std::string> &bxmlTag,
                           const std::string &i) {
  ...
  void visitStringLiteral(const BType &type,
                          const std::vector<std::string> &bxmlTag,
                          const std::string &b) {
  ...
  void visitRealLiteral(const BType &type,
                        const std::vector<std::string> &bxmlTag,
                        const Expr::Decimal &d) {
    stream.OpenElement("Real_Literal");
    stream.PushAttribute(
        "value", std::string(d.integerPart + "." + d.fractionalPart).c_str());
    writeExprAttributes(type, bxmlTag, stream, typeInfos);
    stream.CloseElement();  // Real_Literal
  }
  void visitIdent(const BType &type, const std::vector<std::string> &bxmlTag,
                  const VarName &v) {
    stream.OpenElement("Id");
  ...
  void visitUnaryExpression(const BType &type,
                            const std::vector<std::string> &bxmlTag,
  ...
  void visitBinaryExpression(const BType &type,
                             const std::vector<std::string> &bxmlTag,
  ...
  void visitTernaryExpression(const BType &type,
                              const std::vector<std::string> &bxmlTag,
  ...
  void visitNaryExpression(const BType &type,
                           const std::vector<std::string> &bxmlTag,
                           Expr::NaryOp op, const std::vector<Expr> &vec) {
    stream.OpenElement("Nary_Exp");
    stream.PushAttribute("op", Expr::to_string(op).c_str());
    writeExprAttributes(type, bxmlTag, stream, typeInfos);
    for (auto &e : vec) e.accept(*this);
    stream.CloseElement();  // Nary_Exp
  }
  void visitBooleanExpression(const BType &type,
                              const std::vector<std::string> &bxmlTag,
                              const Pred &p) {
  ...
  void visitRecord(const BType &type, const std::vector<std::string> &bxmlTag,
  ...
  void visitStruct(const BType &type, const std::vector<std::string> &bxmlTag,
                   const std::vector<std::pair<std::string, Expr>> &fds) {
  ...
  void visitQuantifiedExpr(const BType &type,
                           const std::vector<std::string> &bxmlTag,
    ...
    for (auto &id : vars) writeTypedVar(stream, typeInfos, id);
  ...
  void visitQuantifiedSet(const BType &type,
                          const std::vector<std::string> &bxmlTag,
                          const std::vector<TypedVar> vars, const Pred &cond) {
  ...
  void visitRecordUpdate(const BType &type,
                         const std::vector<std::string> &bxmlTag,
  ...
  void visitRecordAccess(const BType &type,
                         const std::vector<std::string> &bxmlTag,
                         const Expr &rec, const std::string &label) {
