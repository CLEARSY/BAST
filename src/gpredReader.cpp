...
GPred readGPredicate(const tinyxml2::XMLElement *dom,
  ...
  if (nullptr == dom) throw GPredReaderException("Null dom element.");

  const std::string tagName = dom->Name();

  auto it = ptags.find(tagName);
  if (it == ptags.end())
    throw GPredReaderException("Unexpected tag '" + tagName + "'.");

  switch (it->second) {
    case GPred::Kind::NotSubNot: {
      const tinyxml2::XMLElement *child = dom->FirstChildElement();
      if (nullptr == child || 0 != strcmp(child->Name(), "Sub_Calculus"))
      ...
      if (nullptr == _not || 0 != strcmp(_not->Name(), "Not"))
        throw GPredReaderException("Not element expected.");
      const tinyxml2::XMLElement *prd = _not->FirstChildElement();
      return GPred::makeNotSubNot(readSubstitution(sub, typeInfos),
                                  readPredicate(prd, typeInfos));
    }

    case GPred::Kind::Sub: {
      const tinyxml2::XMLElement *fst = dom->FirstChildElement();
      const tinyxml2::XMLElement *snd = fst->NextSiblingElement();
      bool overflow = (dom->Attribute("overflow") != nullptr &&
                       0 == strcmp(dom->Attribute("overflow"), "true"));
      return GPred::makeSub(readSubstitution(fst, typeInfos),
                            readGPredicate(snd, typeInfos), overflow);
    }
    ...
    case GPred::Kind::Equivalence: {
      ...
      const tinyxml2::XMLElement *snd = fst->NextSiblingElement();
      if (0 == strcmp(op, "=>")) {
        return GPred::makeImplication(readGPredicate(fst, typeInfos),
                                      readGPredicate(snd, typeInfos));
      } else if (0 == strcmp(op, "<=>")) {
        return GPred::makeEquivalence(readGPredicate(fst, typeInfos),
                                      readGPredicate(snd, typeInfos));
      } else {
        throw GPredReaderException("Unknown binary predicate operator '" +
                                   std::string(op) + "'.");
      }
    }
    case GPred::Kind::ExprComparison: {
      ...
      if (it2 != comparisonOp.end())
        return GPred::makeExprComparison(it2->second,
                                         readExpression(fst, typeInfos),
                                         readExpression(snd, typeInfos));
      if (0 == strcmp(op, "/:"))
        return GPred::makeNegationPred(GPred::makeExprComparison(
            Pred::ComparisonOp::Membership, readExpression(fst, typeInfos),
            readExpression(snd, typeInfos)));
      if (0 == strcmp(op, "/<:"))
        return GPred::makeNegationPred(GPred::makeExprComparison(
            Pred::ComparisonOp::Subset, readExpression(fst, typeInfos),
            readExpression(snd, typeInfos)));
      if (0 == strcmp(op, "/<<:"))
        return GPred::makeNegationPred(GPred::makeExprComparison(
            Pred::ComparisonOp::Strict_Subset, readExpression(fst, typeInfos),
            readExpression(snd, typeInfos)));
      if (0 == strcmp(op, "/="))
        return GPred::makeNegationPred(GPred::makeExprComparison(
            Pred::ComparisonOp::Equality, readExpression(fst, typeInfos),
            readExpression(snd, typeInfos)));
      throw GPredReaderException("Unknown comparison operator '" +
                                 std::string(op) + "'.");
    }
    case GPred::Kind::Forall:
    case GPred::Kind::Exists: {
      const auto op = dom->Attribute("type");
      ...
      if (0 == strcmp(op, "!")) {
        return GPred::makeForall(
            vec,
            readGPredicate(dom->FirstChildElement("Body")->FirstChildElement(),
                           typeInfos));
      } else if (0 == strcmp(op, "#")) {
        return GPred::makeExists(
            vec,
            readGPredicate(dom->FirstChildElement("Body")->FirstChildElement(),
                           typeInfos));
      } else {
        throw GPredReaderException("Unknown type of quantified predicate '" +
                                   std::string(op) + "'.");
      }
    }
    case GPred::Kind::Negation: {
      const auto op = dom->Attribute("op");
      if (0 != strcmp(op, "not"))
        throw GPredReaderException("Unknown unary predicate operator '" +
                                   std::string(op) + "'.");
    ...
    case GPred::Kind::Disjunction: {
      ...
      if (0 == strcmp(op, "&")) {
        ...
        throw GPredReaderException("Unknown n-ary predicate operator '" +
                                   std::string(op) + "'.");
      }
    }
    case GPred::Kind::TaggedPred: {
      auto elt = dom->FirstChildElement();
      return GPred::makeTaggedPred(dom->Attribute("goalTag"),
                                   readGPredicate(elt, typeInfos));
    }
    case GPred::Kind::LetFreshId: {
      auto elt = dom->FirstChildElement();
      return GPred::makeLetFreshId(dom->Attribute("name"),
                                   readGPredicate(elt, typeInfos));
    }
  };
