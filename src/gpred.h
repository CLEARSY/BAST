...
class GPred {
 public:
  ...
  static GPred makeExprComparison(Pred::ComparisonOp op, Expr &&lhs,
                                  Expr &&rhs);
...
class GPred::ImplicationPred : public AbstractGPred {
 public:
  ImplicationPred(GPred &&lhs, GPred &&rhs)
...
class GPred::EquivalencePred : public AbstractGPred {
 public:
  EquivalencePred(GPred &&lhs, GPred &&rhs)
      : lhs{std::move(lhs)}, rhs{std::move(rhs)} {};
...
class GPred::NegationPred : public AbstractGPred {
 public:
  ...
  GPred content;
  void getAllVars(std::set<VarName> &) const { content.getAllVars(); }
  void substFreshId(const std::string &id, const VarName &v) {
...
class GPred::ConjunctionPred : public AbstractGPred {
 public:
  ConjunctionPred(std::vector<GPred> &&content)
      : content{std::move(content)} {};
  void accept(Visitor &v) const { v.visitConjunction(*this); }
  Kind getKind() const { return Kind::Conjunction; }
  size_t hash_combine(size_t seed) const {
    for (auto &p : content) seed = p.hash_combine(seed);
    return seed;
  }
  std::vector<GPred> content;
  void getAllVars(std::set<VarName> &accu) const {
    for (auto &p : content) p.getAllVars(accu);
  }
  void substFreshId(const std::string &id, const VarName &v) {
    for (auto &p : content) p.substFreshId(id, v);
  }
};
class GPred::DisjunctionPred : public AbstractGPred {
 public:
  DisjunctionPred(std::vector<GPred> &&content)
      : content{std::move(content)} {};
...
class GPred::ForallPred : public AbstractGPred {
 public:
  ForallPred(const std::vector<TypedVar> &vars, GPred &&body)
      : vars{vars}, body{std::move(body)} {};
...
class GPred::ExistsPred : public AbstractGPred {
 public:
  ExistsPred(const std::vector<TypedVar> &vars, GPred &&body)
      : vars{vars}, body{std::move(body)} {};
...
class GPred::TaggedPred : public AbstractGPred {
 public:
  TaggedPred(const std::string &tag, GPred &&p)
      : tag{tag}, content{std::move(p)} {};
  void accept(Visitor &v) const { v.visitTaggedPred(*this); }
  Kind getKind() const { return Kind::TaggedPred; }
  size_t hash_combine(size_t seed) const {
    return hashUtil::hash_combine_string(tag, content.hash_combine(seed));
  }
...
class GPred::Sub : public AbstractGPred {
 public:
  Sub(Subst &&subst, GPred &&pred, bool overflow)
      : sub{std::move(subst)}, pred{std::move(pred)}, overflow{overflow} {};
  void accept(Visitor &v) const { v.visitSub(*this); }
  Kind getKind() const { return Kind::Sub; }
  size_t hash_combine(size_t seed) const {
    return pred.hash_combine(sub.hash_combine(seed));
  }
  Subst sub;
  GPred pred;
  const bool overflow;
...
class GPred::NotSubNot : public AbstractGPred {
 public:
  NotSubNot(Subst &&subst, Pred &&pred)
      : sub{std::move(subst)}, pred{std::move(pred)} {};
...
class GPred::LetFreshId : public AbstractGPred {
 public:
  LetFreshId(const std::string &id, GPred &&pred)
      : pred{std::move(pred)}, id{id} {};
  ...
  void substFreshId(const std::string &id2, const VarName &v) {
    if (id != id2) pred.substFreshId(id2, v);
  }
};
