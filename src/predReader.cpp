...
Pred readPredicate(const tinyxml2::XMLElement *dom,
  ...
  if (nullptr == dom) throw PredReaderException("Null dom element.");
  ...
  switch (it->second) {
    ...
    case Pred::PKind::Equivalence: {
      ...
      if (0 == strcmp(op, "=>")) {
        return Pred::makeImplication(readPredicate(fst, typeInfos),
                                     readPredicate(snd, typeInfos));
      } else if (0 == strcmp(op, "<=>")) {
        return Pred::makeEquivalence(readPredicate(fst, typeInfos),
                                     readPredicate(snd, typeInfos));
      } else {
        throw PredReaderException("Unknown binary predicate operator '" +
                                  std::string(op) + "'.");
      }
    }
    case Pred::PKind::ExprComparison: {
      ...
      if (it2 != comparisonOp.end())
        return Pred::makeExprComparison(it2->second,
      ...
      if (0 == strcmp(op, "/="))
        return Pred::makeNegation(Pred::makeExprComparison(
            ...
            readExpression(snd, typeInfos)));
      throw PredReaderException("Unknown comparison operator '" +
                                std::string(op) + "'.");
    }
    ...
    case Pred::PKind::Exists: {
      ...
      for (const tinyxml2::XMLElement *ce = vars->FirstChildElement("Id");
           nullptr != ce; ce = ce->NextSiblingElement("Id")) {
        vec.push_back(VarNameFromId(ce, typeInfos));
      }

      if (quantifier == "!") {
        return Pred::makeForall(
            vec,
            readPredicate(dom->FirstChildElement("Body")->FirstChildElement(),
                          typeInfos));
      } else if (quantifier == "#") {
        return Pred::makeExists(
            vec,
            readPredicate(dom->FirstChildElement("Body")->FirstChildElement(),
                          typeInfos));
      } else
        throw PredReaderException("Unknown type of quantified predicate '" +
                                  std::string(quantifier) + "'.");
    }
    case Pred::PKind::Negation: {
      if (0 != strcmp(op, "not"))
        throw PredReaderException("Unknown unary predicate operator '" +
                                  std::string(op) + "'.");
    ...
    case Pred::PKind::Disjunction: {
      ...
      if (0 == strcmp(op, "&")) {
        ...
        throw PredReaderException("Unknown n-ary predicate operator '" +
                                  std::string(op) + "'.");
      }
    }
  ...
  assert(false);  // unreachable
  throw PredReaderException("Unreachable code executed");
}
}  // namespace Xml
