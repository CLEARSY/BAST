 ...
 public:
  IntegerLiteral(const std::string &s) : value{s} {
    ...
    if (s[0] == '-') {
      assert(s.size() > 1);
      assert(s[1] != '0');
      for (size_t i = 1; i < s.size(); i++) assert(isdigit(s[i]));
    } else if (s[0] == '0') {
      assert(s.size() == 1);
    } else {
      for (size_t i = 0; i < s.size(); i++) assert(isdigit(s[i]));
    }
  }
  ~IntegerLiteral() {}
  ...
  IntegerLiteral *copy() const { return new IntegerLiteral(value); }
  void subst(const std::map<VarName, Expr> &) {}
  void alpha(const std::map<VarName, VarName> &) {}
  void getFreeVars(const std::set<VarName> &, const std::set<VarName> &,
                   std::set<VarName> &) const {}
  void getFreeVars(const std::set<VarName> &, std::set<VarName> &) const {}
  void getFreeTVars(const std::set<VarName> &, std::set<TypedVar> &,
                    const BType &) const {}
...
class Expr::StringLiteral : public ExprDesc {
 public:
  ...
  StringLiteral *copy() const { return new StringLiteral(value); }
  void subst(const std::map<VarName, Expr> &) {}
  void alpha(const std::map<VarName, VarName> &) {}
  void getFreeVars(const std::set<VarName> &, const std::set<VarName> &,
                   std::set<VarName> &) const {}
...
class Expr::RealLiteral : public ExprDesc {
 public:
  ...
  const Decimal value;
  size_t hash_combine(size_t seed) const {
    return hashUtil::hash_combine_string(
        value.integerPart,
        hashUtil::hash_combine_string(value.fractionalPart, seed));
  }
  RealLiteral *copy() const { return new RealLiteral(value); }
  void subst(const std::map<VarName, Expr> &) {}
  void alpha(const std::map<VarName, VarName> &) {}
  void getFreeVars(const std::set<VarName> &, const std::set<VarName> &,
                   std::set<VarName> &) const {}
...
class Expr::IdentExpr : public ExprDesc {
 public:
  ...
  ~IdentExpr() {};
  VarName value;
  size_t hash_combine(size_t seed) const { return value.hash_combine(seed); }
  IdentExpr *copy() const { return new IdentExpr(value); }
  void subst(const std::map<VarName, Expr> &) {
  ...
  void getFreeVars(const std::set<VarName> &boundVars,
                   const std::set<VarName> &freeVars,
                   std::set<VarName> &freeVarsThis) const {
    if (boundVars.find(value) != boundVars.end()) return;
    if (freeVars.find(value) != freeVars.end()) return;
    freeVarsThis.insert(value);
  }
  void getFreeVars(const std::set<VarName> &boundVars,
                   std::set<VarName> &accu) const {
    if (boundVars.find(value) == boundVars.end()) accu.insert(value);
  }
  void getFreeTVars(const std::set<VarName> &boundVars,
                    std::set<TypedVar> &accu, const BType &ty) const {
    if (boundVars.find(value) == boundVars.end()) accu.insert({value, ty});
  }
  void getAllVars(std::set<VarName> &accu) const { accu.insert(value); }
...
class Expr::BooleanExpr : public Expr::ExprDesc {
 public:
  ...
  ~BooleanExpr() {};
  Pred pred;
  size_t hash_combine(size_t seed) const { return pred.hash_combine(seed); }
  BooleanExpr *copy() const { return new BooleanExpr(pred.copy()); }
  void subst(const std::map<VarName, Expr> &map) { pred.subst(map); }
  void alpha(const std::map<VarName, VarName> &map) { pred.alpha(map); }
  void getFreeVars(const std::set<VarName> &boundVars,
                   const std::set<VarName> &freeVars,
                   std::set<VarName> &freeVarsThis) const {
  ...
  void getFreeTVars(const std::set<VarName> &boundVars,
                    std::set<TypedVar> &accu, const BType &) const {
    pred.getFreeTVars(boundVars, accu);
  }
  void getAllVars(std::set<VarName> &accu) const { pred.getAllVars(accu); }
  void substFreshId(const std::string &id, const VarName &v) {
...
void Expr::accept(Visitor &visitor) const {
  switch (tag) {
    ...
    case EKind::IntegerLiteral: {
      visitor.visitIntegerLiteral(type, bxmlTag,
                                  static_cast<IntegerLiteral &>(*desc).value);
      break;
    }
    case EKind::StringLiteral: {
      visitor.visitStringLiteral(type, bxmlTag,
                                 static_cast<StringLiteral &>(*desc).value);
      break;
    }
    case EKind::RealLiteral: {
      visitor.visitRealLiteral(type, bxmlTag,
                               static_cast<RealLiteral &>(*desc).value);
      break;
    }
    case EKind::UnaryExpr: {
    ...
    case EKind::BooleanExpr: {
      visitor.visitBooleanExpression(type, bxmlTag,
                                     static_cast<BooleanExpr &>(*desc).pred);
      break;
    }
    case EKind::Struct: {
      visitor.visitStruct(type, bxmlTag,
                          static_cast<StructExpr &>(*desc).fields);
      break;
    }
    case EKind::Record: {
      visitor.visitRecord(type, bxmlTag,
                          static_cast<RecordExpr &>(*desc).fields);
      break;
    }
...
Expr Expr::makeInteger(const std::string &i,
                       const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::IntegerLiteral, new IntegerLiteral(i), BType::INT,
              bxmlTag);
}

Expr Expr::makeString(const std::string &s,
                      const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::StringLiteral, new StringLiteral(s), BType::STRING,
              bxmlTag);
}

Expr Expr::makeReal(const Decimal &d, const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::RealLiteral, new RealLiteral(d), BType::REAL, bxmlTag);
}

Expr Expr::makeIdent(const VarName &id, const BType &type,
                     const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::Id, new IdentExpr(id), type, bxmlTag);
}

Expr Expr::makePredecessor(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::Predecessor, nullptr, BType::RELATION_INTEGER, bxmlTag);
}
Expr Expr::makeSuccessor(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::Successor, nullptr, BType::RELATION_INTEGER, bxmlTag);
}
Expr Expr::makeEmptySet(const BType &type,
                        const std::vector<std::string> &bxmlTag) {
...
Expr Expr::makeMinInt(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::MinInt, nullptr, BType::INT, bxmlTag);
}
Expr Expr::makeINTEGER(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::INTEGER, nullptr, BType::POW_INT, bxmlTag);
}
Expr Expr::makeNATURAL(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::NATURAL, nullptr, BType::POW_INT, bxmlTag);
}
Expr Expr::makeNATURAL1(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::NATURAL1, nullptr, BType::POW_INT, bxmlTag);
}
Expr Expr::makeINT(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::INT, nullptr, BType::POW_INT, bxmlTag);
}
Expr Expr::makeNAT(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::NAT, nullptr, BType::POW_INT, bxmlTag);
}
Expr Expr::makeNAT1(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::NAT1, nullptr, BType::POW_INT, bxmlTag);
}
Expr Expr::makeSTRING(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::STRING, nullptr, BType::POW_STRING, bxmlTag);
}
Expr Expr::makeBOOL(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::BOOL, nullptr, BType::POW_BOOL, bxmlTag);
}
Expr Expr::makeTRUE(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::TRUE, nullptr, BType::BOOL, bxmlTag);
}
Expr Expr::makeFALSE(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::FALSE, nullptr, BType::BOOL, bxmlTag);
}
Expr Expr::makeREAL(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::REAL, nullptr, BType::POW_REAL, bxmlTag);
}
Expr Expr::makeFLOAT(const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::FLOAT, nullptr, BType::POW_FLOAT, bxmlTag);
}

Expr Expr::makeBinaryExpr(BinaryOp op, Expr &&lhs, Expr &&rhs,
                          const BType &type,
                          const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::BinaryExpr,
              new BinaryExpr(op, std::move(lhs), std::move(rhs)), type,
              bxmlTag);
}

Expr Expr::makeTernaryExpr(TernaryOp op, Expr &&fst, Expr &&snd, Expr &&thd,
                           const BType &type,
                           const std::vector<std::string> &bxmlTag) {
  return Expr(
      EKind::TernaryExpr,
      new TernaryExpr(op, std::move(fst), std::move(snd), std::move(thd)), type,
      bxmlTag);
}

Expr Expr::makeUnaryExpr(UnaryOp op, Expr &&e, const BType &type,
                         const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::UnaryExpr, new UnaryExpr(op, std::move(e)), type, bxmlTag);
}

Expr Expr::makeNaryExpr(NaryOp op, std::vector<Expr> &&vec, const BType &type,
                        const std::vector<std::string> &bxmlTag) {
...
Expr Expr::makeBooleanExpr(Pred &&p, const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::BooleanExpr, new BooleanExpr(std::move(p)), BType::BOOL,
              bxmlTag);
}

Expr Expr::makeRecord(std::vector<std::pair<std::string, Expr>> &&fds,
                      const BType &type,
                      const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::Record, new RecordExpr(std::move(fds)), type, bxmlTag);
}

Expr Expr::makeStruct(std::vector<std::pair<std::string, Expr>> &&fds,
                      const BType &type,
                      const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::Struct, new StructExpr(std::move(fds)), type, bxmlTag);
}

Expr Expr::makeQuantifiedExpr(QuantifiedOp op, const std::vector<TypedVar> vars,
                              Pred &&cond, Expr &&body, const BType &type,
                              const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::QuantifiedExpr,
              new QuantifiedExpr(op, vars, std::move(cond), std::move(body)),
              type, bxmlTag);
}

Expr Expr::makeQuantifiedSet(const std::vector<TypedVar> vars, Pred &&cond,
                             const BType &type,
                             const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::QuantifiedSet, new QuantifiedSet(vars, std::move(cond)),
              type, bxmlTag);
}

Expr Expr::makeRecordFieldUpdate(Expr &&rec, const std::string &label,
                                 Expr &&value, const BType &type,
                                 const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::Record_Field_Update,
              new RecordUpdateExpr(std::move(rec), label, std::move(value)),
              type, bxmlTag);
}

Expr Expr::makeRecordFieldAccess(Expr &&rec, const std::string &label,
                                 const BType &type,
                                 const std::vector<std::string> &bxmlTag) {
  return Expr(EKind::Record_Field_Access,
              new RecordAccessExpr(std::move(rec), label), type, bxmlTag);
}
...
int compare_field_vec(const std::vector<std::pair<std::string, Expr>> &lhs,
                      const std::vector<std::pair<std::string, Expr>> &rhs) {
...
int Expr::vec_compare(const std::vector<Expr> &lhs,
...
size_t Expr::hash_combine(size_t seed) const {
  if (desc != nullptr) seed = desc->hash_combine(seed);
  return hashUtil::hash_combine_int(static_cast<int>(tag), seed);
}

std::string Expr::to_string(UnaryOp op) {
  switch (op) {
    case UnaryOp::IMaximum:
      return "imax";
    case UnaryOp::IMinimum:
      return "imin";
    case UnaryOp::RMaximum:
      return "rmax";
    case UnaryOp::RMinimum:
      return "rmin";
    case UnaryOp::Cardinality:
      return "card";
    case UnaryOp::Domain:
      return "dom";
    case UnaryOp::Range:
      return "ran";
    case UnaryOp::Subsets:
      return "POW";
    case UnaryOp::Non_Empty_Subsets:
      return "POW1";
    case UnaryOp::Finite_Subsets:
      return "FIN";
    case UnaryOp::Non_Empty_Finite_Subsets:
      return "FIN1";
    case UnaryOp::Union:
      return "union";
    case UnaryOp::Intersection:
      return "inter";
    case UnaryOp::Sequences:
      return "seq";
    case UnaryOp::Non_Empty_Sequences:
      return "seq1";
    case UnaryOp::Injective_Sequences:
      return "iseq";
    case UnaryOp::Non_Empty_Injective_Sequences:
      return "iseq1";
    case UnaryOp::IMinus:
      return "-i";
    case UnaryOp::RMinus:
    ...
    case UnaryOp::Identity:
      return "id";
    case UnaryOp::Closure:
      return "closure";
    case UnaryOp::Transitive_Closure:
      return "closure1";
    case UnaryOp::Tail:
      return "tail";
    case UnaryOp::Front:
...
std::string Expr::to_string(BinaryOp op) {
  switch (op) {
    case BinaryOp::IMultiplication:
      return "*i";
    case BinaryOp::RMultiplication:
      return "*r";
    case BinaryOp::FMultiplication:
      return "*f";
    case BinaryOp::Cartesian_Product:
      return "*s";
    case BinaryOp::IExponentiation:
      return "**i";
    case BinaryOp::RExponentiation:
      return "**r";
    case BinaryOp::IAddition:
      return "+i";
    case BinaryOp::RAddition:
      return "+r";
    case BinaryOp::FAddition:
      return "+f";
    case BinaryOp::Partial_Functions:
      return "+->";
    case BinaryOp::Partial_Surjections:
      return "+->>";
    case BinaryOp::ISubtraction:
      return "-i";
    case BinaryOp::RSubtraction:
      return "-r";
    case BinaryOp::FSubtraction:
      return "-f";
    case BinaryOp::Set_Difference:
      return "-s";
    case BinaryOp::Total_Functions:
      return "-->";
    case BinaryOp::Total_Surjections:
      return "-->>";
    case BinaryOp::Head_Insertion:
      return "->";
    case BinaryOp::Interval:
      return "..";
    case BinaryOp::IDivision:
      return "/i";
    case BinaryOp::RDivision:
      return "/r";
    case BinaryOp::FDivision:
      return "/f";
    case BinaryOp::Intersection:
      return "/\\";
    case BinaryOp::Head_Restriction:
      return "/|\\";
    case BinaryOp::Composition:
      return ";";
    case BinaryOp::Surcharge:
      return "<+";
    case BinaryOp::Relations:
      return "<->";
    case BinaryOp::Tail_Insertion:
      return "<-";
    case BinaryOp::Domain_Subtraction:
      return "<<|";
    case BinaryOp::Domain_Restriction:
      return "<|";
    case BinaryOp::Partial_Injections:
      return ">+>";
    case BinaryOp::Total_Injections:
      return ">->";
    case BinaryOp::Partial_Bijections:
      return ">+>>";
    case BinaryOp::Total_Bijections:
      return ">->>";
    case BinaryOp::Direct_Product:
      return "><";
    case BinaryOp::Parallel_Product:
      return "||";
    case BinaryOp::Union:
      return "\\/";
    case BinaryOp::Tail_Restriction:
      return "\\|/";
    case BinaryOp::Concatenation:
      return "^";
    case BinaryOp::Modulo:
      return "mod";
    case BinaryOp::Mapplet:
      return "|->";
    case BinaryOp::Range_Restriction:
      return "|>";
    case BinaryOp::Range_Subtraction:
      return "|>>";
    case BinaryOp::Image:
      return "[";
    case BinaryOp::Application:
      return "(";
    case BinaryOp::First_Projection:
      return "prj1";
    case BinaryOp::Second_Projection:
      return "prj2";
    case BinaryOp::Iteration:
      return "iterate";
    case BinaryOp::Const:
      return "const";
    case BinaryOp::Rank:
      return "rank";
    case BinaryOp::Father:
      return "father";
    case BinaryOp::Subtree:
      return "subtree";
    case BinaryOp::Arity:
      return "arity";
  };
...
std::string Expr::to_string(QuantifiedOp op) {
  switch (op) {
    case QuantifiedOp::Lambda:
      return "%";
    case QuantifiedOp::Intersection:
      return "INTER";
    case QuantifiedOp::Union:
      return "UNION";
    case QuantifiedOp::RProduct:
      return "rPI";
    case QuantifiedOp::RSum:
      return "rSIGMA";
    case QuantifiedOp::IProduct:
      return "iPI";
    case QuantifiedOp::ISum:
      return "iSIGMA";
  }
  assert(false);  // unreachable
  return "";
...
std::string Expr::show() const {
  switch (tag) {
    ...
    case EKind::QuantifiedExpr: {
      ...
      for (size_t i = 1; i < q.vars.size(); ++i)
        vars += " " + q.vars[i].name.show();
      return "(" + to_string(q.op) + " (" + vars + ") " + q.cond.show() + " " +
             q.body.show() + ")";
    }
    case EKind::UnaryExpr: {
      auto &u = toUnaryExpr();
    ...
    case EKind::BinaryExpr: {
      auto &b = toBinaryExpr();
      return "(" + to_string(b.op) + " " + b.lhs.show() + " " + b.rhs.show() +
             ")";
    }
    case EKind::TernaryExpr: {
      auto &t = toTernaryExpr();
      return "(" + to_string(t.op) + " " + t.fst.show() + " " + t.snd.show() +
             " " + t.thd.show() + ")";
    }
    case EKind::NaryExpr: {
      auto &n = toNaryExpr();
      std::string accu;
      for (auto &e : n.vec) accu += " " + e.show();
      switch (n.op) {
    ...
    switch (tag) {
      ...
      case EKind::Id: {
        ...
        if (it != map.end()) {
          tag = it->second.tag;
          // type = it->second.type;
          bxmlTag.insert(bxmlTag.end(), it->second.bxmlTag.begin(),
                         it->second.bxmlTag.end());
...
void Expr::alpha(const std::map<VarName, VarName> &map) {
  if (desc != nullptr) desc->alpha(map);
}
bool Expr::isRenamingNeeded(const std::vector<TypedVar> &vars,
                            const std::set<VarName> freeVars) {
  for (auto &v : vars) {
    if (freeVars.find(v.name) != freeVars.end()) return true;
  }
  return false;
}
void Expr::renameVars(std::vector<TypedVar> &vars,
                      const std::set<VarName> freeVars,
  ...
  if (e1.getTag() == e2.getTag()) {
    switch (e1.getTag()) {
      ...
      case Expr::EKind::UnaryExpr: {
        auto &u1 = e1.toUnaryExpr();
        auto &u2 = e2.toUnaryExpr();
        return u1.op == u2.op && alpha_equals(ctx, u1.content, u2.content);
      }
      case Expr::EKind::BinaryExpr: {
        auto &b1 = e1.toBinaryExpr();
        auto &b2 = e2.toBinaryExpr();
        return b1.op == b2.op && alpha_equals(ctx, b1.lhs, b2.lhs) &&
               alpha_equals(ctx, b1.rhs, b2.rhs);
      }
      case Expr::EKind::TernaryExpr: {
        auto &t1 = e1.toTernaryExpr();
        auto &t2 = e2.toTernaryExpr();
        return t1.op == t2.op && alpha_equals(ctx, t1.fst, t2.fst) &&
      ...
      case Expr::EKind::NaryExpr: {
        ...
        if (n1.vec.size() != n2.vec.size()) return false;
        for (size_t i = 0; i < n1.vec.size(); i++) {
          if (!alpha_equals(ctx, n1.vec[i], n2.vec[i])) return false;
        }
        return true;
      }
      case Expr::EKind::BooleanExpr: {
      ...
      case Expr::EKind::Struct: {
        ...
        for (size_t i = 0; i < s1.fields.size(); i++) {
          if (s1.fields[i].first != s2.fields[i].first) return false;
      ...
      case Expr::EKind::QuantifiedSet: {
        auto &s1 = e1.toQuantifiedSet();
        auto &s2 = e2.toQuantifiedSet();
        if (!ctx.push(s1.vars, s2.vars)) return false;
      ...
      case Expr::EKind::Record_Field_Access: {
        auto &t1 = e1.toRecordAccess();
        auto &t2 = e2.toRecordAccess();
        return t1.label == t2.label && alpha_equals(ctx, t1.rec, t2.rec);
      }
    }
  } else {
    return false;
