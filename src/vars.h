...
struct VarName {
  enum class Kind {
    ...
    Tmp  // Temporary Ident. Guarranted to be different from any other ident
         // when created.
  };
  // Constructors
  static VarName makeVarWithoutSuffix(const std::string &p) {
    return VarName(p, -1);
  };
  static VarName makeVar(const std::string &p, int s) {
    assert(s > 0);
    return VarName(p, s);
  };
  ...
  static int vec_compare(const std::vector<VarName> &v1,
                         const std::vector<VarName> &v2);
  ...
  // Misc
  static VarName getFreshVar(const std::string &prefix,
 ...
 private:
  ...
  // Constructor
  VarName(const std::string &p, int s) : m_prefix{mkPrefix(p)}, m_suffix{s} {};
...
struct TypedVar {
  VarName name;
  BType type;
  TypedVar(const VarName &name, const BType &type) : name{name}, type{type} {};

  static int compare(const TypedVar &v1, const TypedVar &v2);
  static int vec_compare(const std::vector<TypedVar> &v1,
                         const std::vector<TypedVar> &v2);
  inline bool operator==(const TypedVar &other) const {
    return compare(*this, other) == 0;
  }
  inline bool operator!=(const TypedVar &other) const {
    return compare(*this, other) != 0;
...
class Context {
 public:
  bool push(const std::vector<TypedVar> &vars1,
            const std::vector<TypedVar> &vars2) {
    if (vars1.size() == vars2.size()) {
      for (size_t i = 0; i < vars1.size(); i++) {
        if (vars1[i].name.prefix() != vars2[i].name.prefix()) return false;
        if (vars1[i].type != vars2[i].type) return false;
      }
  ...
  static void get_pair(const std::vector<std::vector<TypedVar>> vec,
                       const VarName &v, bool &found,
