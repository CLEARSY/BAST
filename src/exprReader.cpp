...
static void splitDecimal(const char *string, std::string *integralPart,
...
const std::map<std::string, Expr::NaryOp> naryExpOp = {
    {"[", Expr::NaryOp::Sequence}, {"{", Expr::NaryOp::Set}};
...
const std::map<std::string, Expr::EKind> constantExpr = {
    {"MAXINT", Expr::EKind::MaxInt},     {"MININT", Expr::EKind::MinInt},
    {"INTEGER", Expr::EKind::INTEGER},   {"NATURAL", Expr::EKind::NATURAL},
    {"NATURAL1", Expr::EKind::NATURAL1}, {"INT", Expr::EKind::INT},
    {"NAT", Expr::EKind::NAT},           {"NAT1", Expr::EKind::NAT1},
    {"STRING", Expr::EKind::STRING},     {"BOOL", Expr::EKind::BOOL},
    {"REAL", Expr::EKind::REAL},         {"FLOAT", Expr::EKind::FLOAT},
    {"TRUE", Expr::EKind::TRUE},         {"FALSE", Expr::EKind::FALSE},
    {"succ", Expr::EKind::Successor},    {"pred", Expr::EKind::Predecessor}};
...
Expr readExpression(const tinyxml2::XMLElement *dom,
  ...
  if (it == etags.end())
    throw ExprReaderException("Unexpected tag '" + tagName + "'.",
                              dom->GetLineNum());
  ...
  switch (it->second) {
    case Expr::EKind::BinaryExpr: {
      ...
      if (it2 == binaryExpOp.end())
        throw ExprReaderException(
            "Unknown binary expression operator '" + std::string(op) + "'.",
            dom->GetLineNum());
      ...
      return Expr::makeBinaryExpr(it2->second, std::move(lhs), std::move(rhs),
                                  type, bxmlTag);
    }
    case Expr::EKind::TernaryExpr: {
      ...
      if (it2 == ternaryExpOp.end())
        throw ExprReaderException(
            "Unknown ternary expression operator '" + std::string(op) + "'.",
            dom->GetLineNum());
      const tinyxml2::XMLElement *fst = dom->FirstChildElement();
      const tinyxml2::XMLElement *snd = fst->NextSiblingElement();
      ...
      return Expr::makeTernaryExpr(it2->second, std::move(efst),
                                   std::move(esnd), std::move(ethd), type,
                                   bxmlTag);
    }
    case Expr::EKind::NaryExpr: {
      ...
      if (it2 == naryExpOp.end())
        throw ExprReaderException(
            "Unknown n-ary expression operator '" + std::string(op) + "'.",
            dom->GetLineNum());
      std::vector<Expr> lst;
    ...
    case Expr::EKind::BooleanExpr: {
      return Expr::makeBooleanExpr(
          readPredicate(dom->FirstChildElement(), typeInfos), bxmlTag);
    }
    ...
    case Expr::EKind::RealLiteral: {
      ...
      if (!integralPart.empty()) lst.push_back(integralPart);
      if (!decimalPart.empty()) lst.push_back(decimalPart);
      if (lst.size() == 1 || lst.size() == 2) {
        ...
        throw ExprReaderException("Incorrect decimal value (" +
                                      std::string(dom->Attribute("value")) +
                                      ").",
                                  dom->GetLineNum());
      }
    }
    ...
    case Expr::EKind::QuantifiedExpr: {
      ...
      if (it2 == quantifiedExprOp.end())
        throw ExprReaderException(
            "Unknown type of quantified expression '" + std::string(op) + "'.",
            dom->GetLineNum());

      const tinyxml2::XMLElement *vars = dom->FirstChildElement("Variables");
      if (nullptr == vars)
        throw ExprReaderException(
            "The 'Quantified_Exp' element is missing some 'Variables' child.",
            dom->GetLineNum());
      std::vector<TypedVar> ids;
      for (const tinyxml2::XMLElement *ce = vars->FirstChildElement();
           nullptr != ce; ce = ce->NextSiblingElement()) {
        ids.push_back(VarNameFromId(ce, typeInfos));
      }
      Pred pre = readPredicate(
          dom->FirstChildElement("Pred")->FirstChildElement(), typeInfos);
      Expr body = readExpression(
          dom->FirstChildElement("Body")->FirstChildElement(), typeInfos);
      return Expr::makeQuantifiedExpr(it2->second, ids, std::move(pre),
                                      std::move(body), type, bxmlTag);
    }
    case Expr::EKind::QuantifiedSet: {
      const tinyxml2::XMLElement *vars = dom->FirstChildElement("Variables");
      if (nullptr == vars)
        throw ExprReaderException(
            "The 'Quantified_Set' element is missing some 'Variables' child.",
            dom->GetLineNum());
      std::vector<TypedVar> ids;
      for (const tinyxml2::XMLElement *ce = vars->FirstChildElement();
           nullptr != ce; ce = ce->NextSiblingElement()) {
    ...
    case Expr::EKind::UnaryExpr: {
      ...
      if (0 == strcmp(op, "succ")) {
        return Expr::makeBinaryExpr(
            Expr::BinaryOp::Application, Expr::makeSuccessor(),
            readExpression(dom->FirstChildElement(), typeInfos), type, bxmlTag);
      } else if (0 == strcmp(op, "pred")) {
        return Expr::makeBinaryExpr(
            Expr::BinaryOp::Application, Expr::makePredecessor(),
            readExpression(dom->FirstChildElement(), typeInfos), type, bxmlTag);
      } else {
        auto it2 = unaryExpOp.find(op);
        if (it2 == unaryExpOp.end())
          throw ExprReaderException(
              "Unknown unary expression operator '" + std::string(op) + "'.",
              dom->GetLineNum());
        Expr content = readExpression(dom->FirstChildElement(), typeInfos);
        return Expr::makeUnaryExpr(it2->second, std::move(content), type,
                                   bxmlTag);
      }
    }
    case Expr::EKind::Struct: {
      std::vector<std::pair<std::string, Expr>> vec;
      for (const tinyxml2::XMLElement *recItem =
               dom->FirstChildElement("Record_Item");
           nullptr != recItem;
           recItem = recItem->NextSiblingElement("Record_Item")) {
        if (nullptr == recItem->Attribute("label"))
          throw ExprReaderException(
              "The 'Record_Item' element is missing a 'label' attribute.",
              dom->GetLineNum());
    ...
    case Expr::EKind::TRUE:  // Boolean_Literal
    {
      ...
      if (strEqCaseInsensitive(lt, "TRUE"))
        return Expr::makeTRUE(bxmlTag);
      else if (strEqCaseInsensitive(lt, "FALSE"))
        return Expr::makeFALSE(bxmlTag);
      else
        throw ExprReaderException(
            "Unknown boolean literal '" + std::string(lt) + "'.",
            dom->GetLineNum());
    }
    case Expr::EKind::Record_Field_Update: {
      ...
      return Expr::makeRecordFieldUpdate(std::move(rec), label, std::move(fval),
                                         type, bxmlTag);
    }
...
void splitDecimal(const char *str, std::string *integralPart,
                  std::string *decimalPart) {
  unsigned pos1 = 0;
  const char *p = str;
  while (*p != '\0' && *p != '.') {
